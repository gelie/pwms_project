...
| `attempts` | delivery attempts made so far (the queue retries a failure) |
...
An `email` row is delivered by a queued background task, written **in the same
transaction as the row itself** (`pwms.tasks`; see
[Roadmap § Email & notifications](./Roadmap%20&%20Planned%20Integrations.md)).
`attempts` counts the attempts and `error` carries the reason: while the row is
still `pending` it is the last failed attempt (the queue will try again), and
once the attempts are spent the row is `failed` and `error` is why it was given
up on. Migration `0034_notification_attempts`.
...
## 9. Reporting — `ReportShare(BaseModel)`

A report is normally built on the fly from the filters in the query string, so
sharing one needs a way to hold that filter set still. `ReportShare` pins it to a
row under an unguessable token; the row is also the link's audit trail.
Migration `0032_reportshare`. Built by `pwms.reporting.sharing`.

| Field | Notes |
| --- | --- |
| `token` (unique, indexed) | URL-safe token (~43 chars) the share opens with; generated by `generate_token()` |
| `title` | human label (defaults to the report title) |
| `filters` (JSON) | the `ReportFilters.to_dict()` the share was minted with |
| `created_by` (FK `User`, `SET_NULL`, nullable) | who minted it; a share with no creator stops resolving to data |
| `recipients` | comma-separated addresses the link was emailed to |
| `message` | note sent with the link |
| `schedule` | `none` / `daily` / `weekly` / `monthly` — how often the share repeats |
| `schedule_format` | document attached to each scheduled send (`xlsx` / `pdf` / `html` / `csv`), blank = link only |
| `next_send_at` (nullable) | when the next scheduled send falls due |
| `last_sent_at` (nullable), `send_count` | when a scheduled send last ran, and how many have |
| `last_error` | why the last scheduled send failed, if it did |
| `expires_at` (nullable) | when the link stops working (null = never) |
| `revoked_at` (nullable) | when it was withdrawn |
| `access_count`, `last_accessed_at` | open auditing |

A share re-renders **the creator's** figures (`report_for_share()` builds the
report as `created_by`), so a link cannot widen to the reader's access. Helpers:
`is_active` (`not revoked and not expired`), `is_expired`, `revoke()`
(idempotent), `record_access()`, `recipient_list` (splits `recipients` on `,` or
`;`). A revoked, expired or creator-less share answers **410 Gone**; an unknown
token is **404**.

**Scheduling.** `is_scheduled` (a real schedule, not `none`), `is_due(now)`
(active + repeating + `next_send_at` reached), `advance_schedule(after)` and
`record_delivery(sent=…, error=…, when=…)`, which writes the outcome and the next
send time in one `update_fields` so a crash between them cannot lose either.
`next_occurrence(after, schedule)` steps a calendar month for `monthly` — a
share landing on the 1st keeps landing on the 1st, and a day that overshoots a
short month clamps (31 Jan → 28 Feb). Indexed on `(schedule, next_send_at)` for
the delivery job's "due" query.
